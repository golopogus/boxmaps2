...
func _ready():
	if Globals.game_type == 2:
		$CanvasLayer/score_label.visible = false
	else:
		if Globals.game_type == 1:
			$CanvasLayer/score_label.visible = false
...
func randomize_street():
	if Globals.game_type == 1:
		if len(list_of_streets) - 1 >= 0:
			...
			Globals.wrong = num_incorrect
			Globals.right = num_correct
			get_tree().change_scene_to_file("res://find_it_end.tscn")
...
#			randomize_street()
	...
	if Globals.game_type == 1:
		if street != current_street:
			...
			$Sprite2D3.position = get_global_mouse_position() + Vector2(40,0)
			$AnimationPlayer.play("wrong")
			if current_street not in incorrect_street:
				incorrect_street.append(current_street)
				num_incorrect += 1
				
				
		if street == current_street:
			$Sprite2D.visible = true
			$Sprite2D.position = get_global_mouse_position() + Vector2(40,0)
