...
func _ready():
	if Globals.game_type == 2:
		for box in get_parent().get_node("map").get_children():
		...
		for box in boxes:
			for streets in get_parent().get_node("map/" + str(box)).get_children():
...
func adjust_text(street):
	vis = false
	get_parent().get_node("CanvasLayer").get_node("street_label").visible = false
	#$CanvasLayer/street_label.visible = false
	street = street.replace('_',' ')
	get_parent().get_node("CanvasLayer").get_node("street_label").text = street
...
func adjust_highlight(street,box):
	...
	for boxes in get_parent().get_node("map").get_children():
			if boxes.name == box:
				for streets in get_parent().get_node("map/" + box).get_children():
					if streets.name == street:
						get_parent().get_node("map/" + box + "/" + street + "/" + street + "_sprite").visible = highlight
	...
	if buttons == true:
		if vis == false:
			if Input.is_action_just_pressed("space") or Input.is_action_just_pressed("space"):
				get_parent().get_node("CanvasLayer").get_node("street_label").visible = true
				vis = true
		else:
			if Input.is_action_just_pressed("space") or Input.is_action_just_pressed("space"):
				adjust_highlight(current_street,current_box)
